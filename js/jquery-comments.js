...
(function (factory) {
    ...
    var Comments = {
        ...
        events: {
            ...
            'click .pagination-container .pagination-circle:not(.is-current)': 'getPageComments',
        ...
        getDefaultOptions: function () {
            return {
                ...
                getPageComments: function (page, success, error) {
                    success([])
                },
                postComment: function (commentJSON, success, error, audit) {
                    success(commentJSON)
                },
        ...
        fetchDataAndRender: function () {
            ...
            this.options.getComments(function (comments) {
                const {commentsArray, page, pages, commentCount, hasNext, hasPrevious} = comments
                // 创建分页
                self.createPaginationHTML(page, pages, commentCount, hasNext, hasPrevious);
        ...
        getPageComments: function (ev) {
            var self = this;
            var $pageCircle = $(ev.currentTarget);
            var success = function (comments) {
                ...
                self.options.getComments(function () {
                    const {commentsArray, page, pages, commentCount, hasNext, hasPrevious} = comments
                    // 创建分页
                    self.createPaginationHTML(page, pages, commentCount, hasNext, hasPrevious);
            ...
            // console.log($pageCircle.data('page'))
            var page = $pageCircle.data('page');
            if (page === undefined) {
                var current = $('.pagination-circle.is-current').data('page');
                page = $pageCircle.hasClass('pagination-next') ? current + 1 : current - 1
            }
            this.options.getPageComments(page, success);
        },
        ...
        postComment: function (ev) {
            ...
            var audit = function (msg) {
                $('p').remove('.comment-auditing')
                $('.textarea-wrapper').append(`<p class="comment-auditing"><span class="closebtn" onclick="$('p').remove('.comment-auditing')">×</span>${msg}</p>`)
            };
        ...
        // HTML elements
        // =============
        createPaginationHTML: function (page, pages, commentCount, hasNext, hasPrevious) {
            var $page = $('#comments-container');
            ...
            if (hasPrevious) {
                pageList.append(`<li><span class="pagination-circle pagination-previous"> < </span></li>`)
            }

            for (let i = 0; i < pages; i++) {
                if (i === page) {
                    pageList.append(`<li><span class="pagination-circle is-current" data-page="${i}"> ${i + 1} </span></li>`)
                } else {
                    pageList.append(`<li><span class="pagination-circle"  data-page="${i} ">${i + 1}</span></li>`)
                }
            }
            if (hasNext) {
                pageList.append(`<li><span class="pagination-circle pagination-next" > > </span></li>`)
            }
